...
public:
	...
	void set_buffer_data(unsigned int index, const std::vector<float>& stress_values) {
		...
		// Stress Values
		glBindBuffer(GL_ARRAY_BUFFER, mesh_list[index].VBO_stress);
		glBufferData(GL_ARRAY_BUFFER, sizeof(float) * stress_values.size(), &stress_values[0], GL_STATIC_DRAW);
		glEnableVertexAttribArray(3);  // Stress values are passed to location 3 in shader
